...
function ModalExcluirCurso(props) {
  return (
      <div id="ModalExcluirCurso" className='shadown modalExcluirCurso' style={{filter: "blur(0)"}}>
        ...
        <div>
          <button className='botaoNeutro' style={{ paddingInline: 18 }} onClick={() => { excluirCurso(props.id, props.setCursos); props.setModalExcluirCurso(false); props.RemoveBackGroundBlur() }} >Excluir</button>
          <button className='botaoNeutro' onClick={() => { props.setModalExcluirCurso(false); props.RemoveBackGroundBlur() }}>Cancelar</button>
        </div>
      </div>
  )
}
